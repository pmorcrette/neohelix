...
- [ ] Structure-aware cut, copy and paste of a subtree, where pasting adjusts
      the level to where it lands rather than pasting raw text.
- [ ] Clone a subtree a number of times, shifting its timestamps — the standard
      way of creating a recurring set of entries.
- [ ] Copy only the visible text of a region, so a folded outline can be shared
      as an outline.
- [ ] Sort entries, list items or table rows by a chosen key.
- [ ] Dynamic blocks: a block whose contents are regenerated by a named
